...
float unCh1InLast, unCh2InLast, unCh4InLast;
...
#define RC_HIGH_CH 1849
#define RC_LOW_CH 1049
...
float ypr[3] = {0.0f,0.0f,0.0f};           // [yaw, pitch, roll]   yaw/pitch/roll container and gravity vector
...
float velocity, velocityLast;            // global velocity
float va, vb, vc, vd;                    //velocities

// PID Algorithm
#include <PID_v1.h>

#define PITCH_P_VAL 0.0f
...
void setup()
{
  ...
  servoMotorA.attach(ESC1_OUT_PIN);
  servoMotorB.attach(ESC2_OUT_PIN);
  servoMotorC.attach(ESC3_OUT_PIN);
  servoMotorD.attach(ESC4_OUT_PIN);

  delay(100);

  // arm ESCs
  servoMotorA.write(ESC_MIN);
  servoMotorB.write(ESC_MIN);
  servoMotorC.write(ESC_MIN);
  servoMotorD.write(ESC_MIN);
...
void loop()
{
  ...
  if(bUpdateFlagsShared) {
    ...
    if(bUpdateFlags & CH1_FLAG)
    {
      ...
      unCh1In = map(unCh1InShared, RC_LOW_CH, RC_HIGH_CH, ROLL_MIN, ROLL_MAX);
    }
    if(bUpdateFlags & CH2_FLAG)
    {
      //unCh2In = unCh2InShared;
      unCh2In = map(unCh2InShared, RC_LOW_CH, RC_HIGH_CH, PITCH_MIN, PITCH_MAX);
    }
    if(bUpdateFlags & CH3_FLAG)
    {
      //unCh3In = unCh3InShared;
      velocity = map(unCh3InShared, RC_LOW_CH, RC_HIGH_CH, ESC_MIN, ESC_MAX);
    }
  ...
  if((unCh1In < ROLL_MIN) || (unCh1In > ROLL_MAX)) unCh1In = unCh1InLast;
  if((unCh2In < PITCH_MIN) || (unCh2In > PITCH_MAX)) unCh2In = unCh2InLast;
  if((velocity < ESC_MIN) || (velocity > ESC_MAX)) velocity = velocityLast;
  //if((unCh4In < YAW_MIN) || (unCh4In > YAW_MAX)) unCh4In = unCh4InLast;
  ...
  va = abs(((-100+bal_ac)/100)*velocity);
  vb = abs(((-100+bal_bd)/100)*velocity);
  vc = ((100+bal_ac)/100)*velocity;
  vd = ((100+bal_bd)/100)*velocity;
